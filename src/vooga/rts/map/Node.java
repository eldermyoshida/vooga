...
public class Node {
	...
	public Node(int x, int y, int tier) {
		...
		myY = y;
		myTier = tier;
		myBounds = new Rectangle(myX * NODE_SIZE, myY * NODE_SIZE, NODE_SIZE,
				NODE_SIZE);
		/*
		...
		myCenter = new Location3D(myX * NODE_SIZE + NODE_SIZE / 2, myY
				* NODE_SIZE + NODE_SIZE / 2, tier);
	}
	...
	public <T extends GameEntity> List<T> filterGameSprites(
			List<GameSprite> fullList, GameSprite other, int teamID,
			boolean same) {
	...
	public void paint(Graphics2D pen, int index) {
		Point2D screen = Camera.instance().worldToView(
				new Location3D(myX * NODE_SIZE, myY * NODE_SIZE, 0));
		if (myX % 10 == 0 || myY % 10 == 0) {
			pen.fill(new Ellipse2D.Double(screen.getX(), screen.getY(),
					NODE_SIZE, NODE_SIZE));
		} else {
			pen.draw(new Ellipse2D.Double(screen.getX(), screen.getY(),
					NODE_SIZE, NODE_SIZE));
		}
		pen.setColor(Color.red);
		pen.setFont(new Font("Arial", Font.PLAIN, 10));
		pen.drawString(Integer.toString(index), (int) screen.getX(),
				(int) screen.getY());
		pen.setColor(Color.blue);
		pen.setFont(new Font("Arial", Font.PLAIN, 12));
		pen.drawString("(" + myX + ", " + myY + ")", (int) screen.getX(),
				(int) screen.getY() + NODE_SIZE / 2 + 6);
