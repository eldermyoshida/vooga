    ...
    public void paintComponents (Graphics2D pen, ViewDataSource data, Dimension screenSize) {
        ...
        for (int n = 0; n < data.ObjectNumber(); n++) {
            if (!titleMade && data.getPaintable(n) instanceof HUDTitleText) {
                Point2D.Double location = new Point2D.Double(screenSize.width / 2, TITLE_DISTANCE_FROM_TOP);
                data.getPaintable(n).paint(pen, location, data.getSize(n));
                titleMade = true;
            }
            else if (data.getPaintable(n) instanceof HUDText) {
                Point2D.Double location = new Point2D.Double(screenSize.width / 2, screenSize.height / 2 + LINE_HEIGHT * lineCount);
                data.getPaintable(n).paint(pen, location, data.getSize(n));
                lineCount++;
            }
            else {
