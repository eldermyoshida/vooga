...
public class MapLoader extends ObjectLoader {
    ...
    protected void load (String mapName) {
        ...
        for (int i = 0; i < mapNodes.getLength(); i++) {
            ...
            if (mapName.equals(name)) {
                NodeList stateNodes =
                        ((Element) node).getElementsByTagName(getResourceBundle()
                                .getString("State"));
                addStates(stateNodes, myMap);
                myMap.setLocation(new UpdatableLocation(
                                                        ...
                                                        Integer.parseInt(getAttributeValue(node,
                                                                                           getResourceBundle()
                                                                                                   .getString("YSize"))) / 2));
                NodeList startingPosNodes =
                        node.getElementsByTagName(getResourceBundle().getString("StartingPosition"));
                addStartingPositions(startingPosNodes);
                NodeList enviroObjectNodes =
                        node.getElementsByTagName(getResourceBundle()
                                .getString("EnvironmentObject"));
    ...
    private void addStartingPositions (NodeList startingPosNodes) {
        for (int i = 0; i < startingPosNodes.getLength(); i++) {
            ...
            int xCoord =
                    Integer.parseInt(getAttributeValue(startingPosition, getResourceBundle()
                            .getString("XCoordinate")));
            int yCoord =
                    Integer.parseInt(getAttributeValue(startingPosition, getResourceBundle()
                            .getString("YCoordinate")));
    ...
    private void addEnviroObjects (NodeList enviroObjectNodes) {
        for (int i = 0; i < enviroObjectNodes.getLength(); i++) {
            ...
            int xCoord =
                    Integer.parseInt(getAttributeValue(enviroObjectNode, getResourceBundle()
                            .getString("XCoordinate")));
            int yCoord =
                    Integer.parseInt(getAttributeValue(enviroObjectNode, getResourceBundle()
                            .getString("YCoordinate")));
            EnvironmentObject newEnvironmentObject =
                    new EnvironmentObject(
                                          getAttributeValue(enviroObjectNode,
                                                            getResourceBundle()
