...
public class Physics {
     ...
     * @return Two vectors in an array, the first is the new velocity of object A and the second is the new velocity of object B.
     */
    public static Vector[] elasticCollision (Vector aVelocity, int aMass, Vector bVelocity, int bMass) {
     ...
     * @return Two vectors in an array, the first is the new velocity of object A and the second is the new velocity of object B.
     */
    public static Vector[] inelasticCollision (Vector aVelocity, int aMass, Vector bVelocity, int bMass) {
     ...
     * @param restitution The coefficient of restitution where 1 is perfectly elastic and 0 is perfectly inelastic
     * @param aVelocity The velocity of object A
     * @param aMass The mass of object A
     * @param bVelocity The velocity of object B
     * @param bMass The mass of object B
     * @return Two vectors in an array, the first is the new velocity of object A and the second is the new velocity of object B.
     */
    public static Vector[] partiallyInelasticCollision (double restitution, Vector aVelocity, int aMass, Vector bVelocity, int bMass) {
        ...
        double aVectorTurnedDX = (restitution * bMass * (bTurned.getXChange() - aTurned.getXChange()) +
                aMass * aTurned.getXChange() + bMass * bTurned.getXChange()) / (aMass + bMass);
        
        double bVectorTurnedDX = (restitution * aMass * (aTurned.getXChange() - bTurned.getXChange()) +
                aMass * aTurned.getXChange() + bMass * bTurned.getXChange()) / (aMass + bMass);
     ...
     */
    public static Vector gravity (Vector velocity, double gravitationalAcceleration, double terminalSpeed) {
    ...
    /**
     * Applies gravity to an object in contact with something acting as the ground with a particular angle.
     ...
     */
    public static Vector sliding (Vector velocity, double gravitationalAcceleration, double terminalSpeed,
        ...
        //Represents the adjustment to the velocity by one time period. The adjustment scales linearly with how
        //  close the velocities are to matching and inversely with the coefficient of friction, where a higher
        ...
        double gravityDXVelocity = - (Math.sin(Math.toRadians(inclineAngle)) * gravitationalAcceleration) * 
                (terminalSpeed * coefficientOfFriction - velocity.getMagnitude()) * (1 - coefficientOfFriction);
    ...
    /**
     * Returns the velocity vector reflected about a wall of impact. Assumes a perfect elastic collision.
     * @param velocity The velocity of the object
    ...
    public static Vector bounce (Vector velocity, double inclineAngle) {
        Vector wall = new Vector (velocity,Math.cos(Math.toRadians(inclineAngle)),Math.sin(Math.toRadians(inclineAngle)));
     ...
     * @return The new velocity with force applied
     */
    public static Vector applyForce (Vector velocity, double mass, Vector force, double terminalSpeed) {
