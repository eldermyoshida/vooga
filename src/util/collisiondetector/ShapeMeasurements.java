...
public class ShapeMeasurements {
	...
	/**
	 * A class that returns basic measurements of Shapes
	 */
	...
	/**
	 * Uses rectange bounds of each shape to get a quick Point2D for the center.
	 */
	...
	/**
	 * Uses path of bounds to precisely calculate the centroid of the shape.
	 * DISCLAMER: Had to special case for Rectangles :(
	 */
	...
	/**
	 * Uses path of bounds to precisely calculate the centroid of the shape.
	 * DISCLAMER: if the shape has holes (like a donut this method will find the
	 * center to be inside the donut, so use wisely! no rotation considered
	 */
	...
	/**
	 * Treating the shape as a rectangle, returns the location of the new
	 * Location of the past Bottom RightCorner. NOTE: this does not still mean
	 * it will be "Bottom" or "Right" anymore.
	 * 
	 * @param angle
	 *            is the angle ther shape is rotated around the top left corner
	 */
	...
	/**
	 * Treating the shape as a rectangle, returns the location of the Bottom
	 * Right Corner
	 */
	...
	/**
	 * Treating the shape as a rectangle, returns the location of the new
	 * Location of the past BottomLeftCorner. NOTE: this does not still mean it
	 * will be "Bottom" or "Left" anymore.
	 * 
	 * @param angle
	 *            is the angle ther shape is rotated around the top left corner
	 */
	...
	/**
	 * Treating the shape as a rectangle, returns the location of the new
	 * Location of the past TopRightCorner. NOTE: this does not still mean it
	 * will be "Top" or "Right" anymore.
	 * 
	 * @param angle
	 *            is the angle ther shape is rotated around the top left corner
	 */
	...
	/**
	 * Treating the shape as a rectangle, returns the location of the top right
	 * Corner
	 */
	...
	/**
	 * Treating the shape as a rectangle, returns the location of the top left
	 * corner
	 * 
	 * @param angle
	 *            is the angle ther shape is rotated around the top left corner
	 ...
	 * 
	 * @param precision
	 *            represents the definition with which the Boundary is
	 *            described. The higher the precision, the better the
	 *            definition. p = 1000 typically a good value to precisely
	 *            describe a shape
	 */
	public List<Location> makeBoundary(Shape shape1, Double rotationAngle,
			double precision) {
		if (precision == 0)
			precision = BOUNDARY_PRECISION;
		AffineTransform transform1 = AffineTransform.getRotateInstance(Math
				.toRadians(Vector.SanitizeAngle(rotationAngle)));
	...
	/**
	 * Convenience method to create a List of locations that describe a
	 * particular line from loc1 to loc2
	 * 
	 * @param NumberOfPoints
	 *            is the number of points that will be used to describe the
	 *            line.
	 */
	public List<Location> makeBoundaryLine(Location loc1, Location loc2,
			int NumberOfPoints) {
		...
		for (int i = 0; i < NumberOfPoints; i++) {
			...
			x += (vec.getMagnitude()
					* Math.cos(Math.toRadians(vec.getDirection())) / NumberOfPoints);
			y -= (vec.getMagnitude()
					* Math.sin(Math.toRadians(vec.getDirection())) / NumberOfPoints);
		}
		return line;
	}
	 ...
	 * 
	 * @param is
	 *            the angle the rectangle is rotated about its top left corner
	 */
	public List<Location> makeRectangleBoundary(Shape shape1,
	...
	public List<Location> makeRectangularBoundary(Shape shape1) {
		return makeRectangleBoundary(shape1, NO_ROTATION,
				INTERSECT_LINE_PRECISION);
	}

	/**
	 * Returns whether a Locations is within a certain distance of a list of
	 * locations
	 * 
	 * @param precision
	 *            is how close in pixels the point has to be to the boundary
	 */
	public boolean pointOnBoundary(List<Location> line, Location point,
			double precision) {
		for (Location point2 : line) {
			if (Math.abs(point2.distance(point)) < precision)
				return true;
		}
		return false;
	}
	...
	/**
	 * Returns based on tha angle turned, the new Location of a point on an
	 * original rectangle All rotation is about the top left corner
	 */
